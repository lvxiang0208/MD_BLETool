//
//  MD_LTNew2G_DataHandle.swift
//  PelvicFloorPersonal
//
//  Created by Medo on 2021/8/25.
//  Copyright © 2021 henglongwu. All rights reserved.
//

import UIKit

class MD_LTNew2G_DataHandle: NSObject {
    ...
    private lazy var pressureValue_Smooth : MD_DataSmooth = {
        let t_smooth = MD_DataSmooth()
        t_smooth.setSmoothLength(length: 40)
        return t_smooth
    }()
...
//        let quotient:Double = 3000000.0 / (4095.0 * 2000.0)
        ...
        for i in 0...3 {
            smooth_channel_2_value[i] =
            channel_2_Smooth.smoothData(data: original_channel_2_value[i])
...
//        //子版本号
        ...
        if let model = device {
            MD_DispatcherCenter.shared.dispatchLT2rdBleShakeSuccess(model: model, firmwareVersion: UInt8(device?.firmwareVersion ?? 0), bootVersion:  UInt8(device?.hardwareVersion ?? 0))
        }
...
//            device.workStatus = MD_LT3G_DeviceInfo.LT3rdDeviceWorkStatus(rawValue: data[9]) ?? .standBy
//            //信号强度RSSI=RSSI_H*256+RSSI_L-1000，单位dBm；
    ...
    private func dataHandleBy_eppromUpload(data:[UInt8]) {
        ...
        if blockNum == LT_3rdEEPROMNotify.Lanting3GEEPROMBlockTypeModel.rawValue {
            ...
            let serNo = String(format: "%lx%@", device?.deviceM ?? 0xff,serialNumber).uppercased()
            ...
            if (device?.deviceM == 0xFF) {
                return;
            } else if ((device?.serialNumber?.count ?? 0) < 5) {
                //TODO:-- 信息不完整，设备故障
                NSLog("信息不完整，设备故障");
                return;
            }
            ...
            if sumInt8 == data[8] {
                ...
                //调用实现读取激活日期的代理方法
                MD_DispatcherCenter.shared.dispatchLT2rdBleReadEEPROM(device: device ?? MD_BLEModel(), deviceModel: device?.deviceM ?? 0xff, serialNumber: serialNumber, activationDate: activationDate)
            } else {
                //TODO:--没有写过,向服务器申请时间,写入当前日期
                device?.activationDate = nil
                MD_DispatcherCenter.shared.dispatchLT2rdBleReadEEPROM(device: device ?? MD_BLEModel(), deviceModel: device?.deviceM ?? 0xff, serialNumber: serialNumber, activationDate: nil)
            }
        }
...
/*
    ...
    func handleNotifyUpgradeData(tempByte:[UInt8]) {
        ...
        if res == 0x00 {
            ...
            print("hexIndex\(self.hexDatumIndex)and\(data.count)")
    ...
    func upgradeKey(seedData:[UInt8]) {
        let sendData:Data = MD_LT3G_CommandTool.commandUpgradeKey(seedData: seedData);
        device?.writeValueToDevice(data: sendData)
    }
...
//        let path = MD_BLEDeviceDealTool.getLanTingDataPath() + "/lanting2GUpgrade.bin"
        let path = Bundle.main.path(forResource: "lanting_update", ofType: "hex") ?? ""
        do {
            ...
            for (i,hexLine) in hexList.enumerated() {
                if i == 0 {    //第一行特殊处理
                    ...
                    if hexLine.count > 8 {
                        let poS = (hexLine as NSString).substring(with: NSRange(location: hexLine.count - 8, length: 8))
                        for k in stride(from: 0, to: poS.count, by: 2) {
                            if k + 1 < poS.count { //防止越界
                                let hStr = (poS as NSString).substring(with: NSRange(location: k, length: 2))
                ...
                if i == 1 || i == 2 {
                    ...
                    if i == 1 {
                        if hexLine.count > 11 {
                            poS = (hexLine as NSString).substring(with: NSRange(location: 8, length: 4))
                        }
                    } else {
                        if hexLine.count > 5 {
                            poS = (hexLine as NSString).substring(with: NSRange(location: 2, length: 4))
                        }
                    }
                    for k in stride(from: 0, to: poS.count, by: 2) {
                        if k + 1 < poS.count { //防止越界
            ...
            for i in 0...index {
                ...
                if i == index {  //最后一个余数
                    ...
                    let poS = (upgradeFullStr as NSString).substring(with: NSRange(location: i * 256, length: length))
                    for k in stride(from: 0, to: poS.count, by: 2) {
                        if k + 1 < poS.count { //防止越界
                            let hStr = (poS as NSString).substring(with: NSRange(location: k, length: 2))
                            let hHex = UInt(hStr,radix: 16) ?? 0
                    ...
                    let poS = (upgradeFullStr as NSString).substring(with: NSRange(location: i * 256, length: 256))
                    for k in stride(from: 0, to: poS.count, by: 2) {
                        if k + 1 < poS.count { //防止越界
                            let hStr = (poS as NSString).substring(with: NSRange(location: k, length: 2))
                            let hHex = UInt(hStr,radix: 16) ?? 0
